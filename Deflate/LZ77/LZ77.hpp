...
bool LZ::compress()
{
	...
	output.push_back(Triplet(0, 0, (int)search[32767]));
	...
	while(look[0] != '\0')
	{
		...
		if (pos.size() == 0)
			this->output.push_back(Triplet(0, 0, (int)look[0]));
		...
		else
		{
			for (auto itr = pos.begin(); itr != pos.end(); itr++)
			{
				if (itr->follow == j  - 1 && itr->length > 3)
				{
					exchange = itr->length;
					this->output.push_back(Triplet(32768 - itr->distance, itr->length, (int)look[j-1]));
					break;
				}
				else
				{
...
vector<LZ::Triplet> LZ::getOutput()
{
	return this->output;
}